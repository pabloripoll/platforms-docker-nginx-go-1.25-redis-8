...
- [Air *(latest)*](https://github.com/air-verse/air)
<br><br>
...
COMPOSE_PROJECT_IMGK="alpine-3.24-nginx-go-1.25"        # <- real main image keys to manage automations for sharing resources -------------------------------> #
...
- `./docker/config/supervisor/conf.d-sample/go.conf` -> `./docker/config/supervisor/conf.d/go.conf`

The **basic** setting case will run the application once when the service is started and NGIX wont be able to show new changes, despite further `$ go run main.go` command executions
```bash
[program:go]
...
The **recommended** setting is the following: The container compiler installs [Air - Live reload for Go apps](https://github.com/air-verse/air) for refreshing the app, because without it the Supervisor GO service should be reloaded on every change.
```bash
[program:go]
...
`./platform/nginx-go-1.25/docker/config/supervisor/conf.d-sample`

Before building the container you must copy that directory to:
`./platform/nginx-go-1.25/docker/config/supervisor/conf.d`
